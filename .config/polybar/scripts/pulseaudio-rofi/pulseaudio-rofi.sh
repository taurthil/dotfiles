#!/bin/sh
ROFI_THEME='/home/taurthil/.config/rofi/themes/bwmenu.rasi'
outputs() {
    OUTPUT=$(pactl list short sinks | cut  -f 2 | rofi -theme $ROFI_THEME -dmenu -p "Output" -mesg "Select prefered output source" )
...
inputs() {
    INPUT=$(pactl list short sources | cut  -f 2 | grep input | rofi -theme $ROFI_THEME -dmenu -p "Input" -mesg "Select prefered input source" )
